...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var filteredSample = sample.filter(sampleObj => sampleObj.id == sample);
...
// Gauge Chart
    ...
    var filterMetadata = filteredMetadata[0];

    // 3. Create a variable that holds the washing frequency.
    var washingFrequency = filterMetadata.washingFrequency;

    // 4. Create the trace for the gauge chart.
    var gaugeData = [{
      type:"indicator",
      mode: "gauge+number",
      title: {text: "Belly Button Washing Frequency<br><sup>Scrubs per Week</sup>",font: {size: 20} },
      delta: {reference: washingFrequency},
      gauge: {
